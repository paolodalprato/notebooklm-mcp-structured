...
import { CONFIG, NOTEBOOKLM_AUTH_URL, isNotebookUrl } from "../config.js";
...
export class AuthManager {
   ...
   *
   * SIMPLE & RELIABLE: Just wait for URL to change to the notebook host
   */
  ...
  async loginWithCredentials(
    ...
    if (currentUrl.includes("accounts.google.com")) {
      ...
      log.warning("  ⚠️  Reached Gemini Notebook but couldn't detect successful login");
   ...
   *
   * Just checks if URL changes to the notebook host - no complex UI element searching!
  ...
  private async waitForRedirectAfterLogin(
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (isNotebookUrl(currentUrl)) {
          log.success("    ✅ Gemini Notebook URL detected!");
          // Short wait to ensure page is loaded
          await page.waitForTimeout(2000);
          return true;
        }
   ...
   *
   * Just checks if URL is on the notebook host - no complex UI element searching!
   * Matches the simplified approach used in performLogin().
   */
